...
| `API_BASE_URL` | Base URL for the API suite (default: `https://automationexercise.com/api/` — the trailing slash matters, see `apps/api/src/clients/apiClient.ts`) |
...
- **Web & mobile**: each page is a class extending `BasePage`, and exposes action methods (`login()`, `searchProduct()`) and query methods (`getVisibleProductNames()`). Web specs consume page objects through a fixture (`apps/web/src/fixtures/pages.fixture.ts`); mobile specs instantiate page objects directly — there's no fixture layer there, since the driver/session comes from `apps/mobile/wdio.conf.ts` instead.
- **Web locators are centralized**, not inline: each `apps/web/src/pages/*Page.ts` has a matching `apps/web/src/locators/*Page.locators.ts` (e.g. `cartPage.ts` ↔ `cartPage.locators.ts`) exporting a flat, typed (`LocatorObj`) map of selector strings, which the page object turns into `Locator` fields in its constructor. Accessible-role locators (`getByRole`) are the one exception — they stay as direct calls in the page object, since Playwright's string-based `role=` engine doesn't have an equivalent to `getByRole`'s accessible-name matching. Mobile locators stay inline (no separate file there) — this pattern is web-only.
...
- Add a new page/service by following the existing files in the same directory.
...
- **API suite fails only in CI with `415 Unsupported Media Type`, even on `GET` requests**: seen when GitHub Actions' shared datacenter IP ranges get treated more strictly by the site's Cloudflare/WAF layer than a typical dev machine — the request itself is fine (confirmed by it succeeding locally against the same endpoint). `apps/api/playwright.config.ts` sets a browser-like `User-Agent`/`Accept-Language` to reduce this, but it isn't guaranteed to eliminate it since the site's infrastructure isn't under this framework's control. If it recurs, re-running the workflow is worth trying — GitHub Actions assigns a new runner (often a different IP) per run, unlike retries within the same job, which reuse the same runner and will fail identically if the block is IP-based.
- **Live-site copy changes**: product catalog data (`packages/test-data`) is the main exposure — if automationexercise.com's catalog changes, refresh the values from the live site. `loginPage.ts`'s login-error check and `auth.spec.ts`'s API error-message check were both hardened to reduce this risk further: the web check asserts on the login form's red-styled error paragraph structurally rather than its exact wording, and the API check treats `responseCode` (a stable, structured field) as the authoritative signal and only checks that `message` is non-empty rather than matching specific text.
