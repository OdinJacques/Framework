...
- **Web/mobile page objects**: a new page is a class extending `BasePage` in that suite's `src/pages/`, with action methods (`login()`, `searchProduct()`) / query methods (`getVisibleProductNames()`). Web specs consume pages through `apps/web/src/fixtures/pages.fixture.ts`; mobile specs instantiate page objects directly (no fixture layer there) — the driver/session itself is never set up per-spec, it comes from `apps/mobile/wdio.conf.ts` (which imports `wdOpts` — capabilities, host, port — from `apps/mobile/config/android.config.ts`, and wires `services: [['appium', ...]]`), and WebdriverIO's test runner injects the `browser`/`$`/`$$` globals automatically into every spec file.
- **Web locators live in a parallel `apps/web/src/locators/*Page.locators.ts` file per page** (e.g. `pages/productsPage.ts` → `locators/productsPage.locators.ts`), each exporting a flat, typed (`LocatorObj` from `apps/web/src/types.ts`) map of plain CSS/XPath selector strings, e.g. `productsPageLocators.searchInput`. Page objects build `Locator` fields from these in the constructor (`page.locator(productsPageLocators.searchInput)`) rather than embedding the selector string inline — centralizes and reuses selector strings across the file. **Exception**: Playwright's accessible-role locators (`page.getByRole('link', { name: 'Cart' })`) stay as direct API calls in the page object rather than being forced into a locator string — Playwright's `role=` string engine matches on visible text via `>> text=`, not the same accessible-name semantics as `getByRole`, so moving these would be a behavioral change, not just a refactor (verified empirically against the live site). Mobile keeps locators inline (no separate locators file there) — this pattern is web-only.
- **File naming**: `apps/web/src/pages`, `apps/web/src/locators`, and `apps/mobile/src/pages` use camelCase filenames (`basePage.ts`, `networkInternetPage.ts`, `cartPage.locators.ts`) — the descriptive name plus its "kind" word (Page, Client) merge into one camelCase token; a further purpose-suffix beyond that (`.locators`, `.spec`) stays dot-separated. `apps/api/src/clients/apiClient.ts` and `apps/sql/src/dbClient.ts` follow the same rule. Folders not mentioned here (`*/tests/specs` outside mobile, `*/src/services`, `*/src/fixtures`, `sql/db`, `mobile/config`) haven't been converted — don't assume the whole repo follows this scheme.
- **API object model**: a new resource gets a `*.service.ts` in `apps/api/src/services/`, wrapping the shared `ApiClient` (`apps/api/src/clients/apiClient.ts`) and returning typed `@framework/shared-types` entities instead of raw JSON. Wire it into `apps/api/src/fixtures/services.fixture.ts`.
...
- **SQL suite pattern**: a new test calls `createDatabase()` from `apps/sql/src/dbClient.ts` (defaults to an in-memory DB) and, if it needs seeded rows, `seed(db)` from `apps/sql/db/seed.ts` — typically in `beforeEach`/`beforeAll`, so each test gets a fresh, isolated database.
